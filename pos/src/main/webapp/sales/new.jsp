	...
	<tr>
		<td id="register_items_container">
			<table id="title_section">
				<tr>
					<td id="title_icon">
						<img src='${pageContext.request.contextPath}/images/menubar/sales.png' alt='title icon' />
					</td>
					<td id="title">
						Sales Register
					</td>
                ...
                <option value="1" <c:if test="${orderEntity.orderType.id == 1 }" >selected="selected"</c:if>>Sale</option>
                <option value="2" <c:if test="${orderEntity.orderType.id == 2 }" >selected="selected"</c:if>>Return</option>
                <option value="3" <c:if test="${orderEntity.orderType.id == 3 }" >selected="selected"</c:if>>Purchase</option>
						...
						<tr>
                                ...
                                <table>
                                           <tr id="reg_item_top">
                                               ...
                                               <td id="reg_item_name">${item.product.name}</td>
	                                            <td id="reg_item_number"><a href="${pageContext.request.contextPath}/items/edit.ps?id=${item.product.id}" class="thickbox none" >${item.product.id}</a></td>
            ...
            <c:choose>
                 ...
                 <c:when test="${!empty orderEntity.customer}">
                    <div id="customer_info_filled">
		                ...
		                <div id="customer_edit"><a title="Update Customer" class="thickbox none" href="<s:url action="edit" namespace="/customers"/>?id=${orderEntity.customer.id}">Edit</a></div>
               ...
               <c:otherwise>
                  <div id='customer_info_empty'>
							...
							<label id="customer_label" for="customer">
								Select Customer (Optional)							</label>
							<input type="text" name="customerId" value="Type customer name..." id="customer" size="30" accesskey="c"  />							</form>
							<div id="add_customer_info">
								<div id="common_or">
									OR
							    </div>
								<a href="<s:url action="new" namespace="/customers"/>" class="thickbox none" title="New Customer">
								<div class='small_button' style='margin:0 auto;'> <span>New Customer</span> </div></a>
								</div>
				...
				<div id='sale_details'>
					<table id="sales_items">
						...
						<tr>
							...
							<td class="right"><fmt:formatNumber value="${orderEntity.subTotalAmount}"  type="currency"/> </td>
						</tr>
                     ...
                     <c:if test="${ !empty orderEntity.paymentList}">
                      ...
                      <table id="register">
							...
							<th id="pt_amount">Amount</th>
							  ...
							  <form id="edit_payment_formCash" accept-charset="utf-8" method="post" action="${pageContext.request.contextPath}/sales/editPayment.ps/Cash">
							  </form>
						...
						<tr class="">
							...
							<td style="text-align:right; ">
								...
								<div class="float_left" style="text-align:right;font-weight:bold;">
								 <fmt:formatNumber value="${orderEntity.dueAmount}"  type="currency"/> </div>
...
<script type="text/javascript">
      <c:if test="${not empty errorMsg }">
       set_feedback("${errorMsg}",'error_message',false);
       </c:if>
</script>
