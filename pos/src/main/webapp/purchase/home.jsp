	...
	function init_table_sorting()
{
	...
	if($('.tablesorter tbody tr').length >1)
	{
		...
		$("#suspended_sales_table").tablesorter(
		{
	...
	<table id="sortable_table" class="tablesorter">
        ...
        <c:forEach var="item" items="${orderEntityList}">
            <tr>
                        ...
                        <td>  <a title="Customer" class="thickbox" href="<s:url action="edit" namespace="/suppliers" />?id=${item.customer.id}">${item.customer.name},${item.customer.city}</a></td>
						<td>
